...
class Experiment:
    def __init__(self, client, secret, experiment_id):
        ...
        self.model, self.config = self.get_model_instance()
        self.client=client
    ...
    def get_model_instance(self):
        ...
        for row in responses:  # pseudo stream
            ...
            config = self._set_custom_training_config(config, self.client)  # todo allow to change everything

            if 'dataset' in config["training"]:  # new data wrapper
                data_generator = NewWrapper(config, self.client)
            else:
                data_generator = OldWrapper(config, self.client)
